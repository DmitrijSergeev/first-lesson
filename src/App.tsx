...
export function App() {
    ...
    const [selectedTracksIds, setSelectedTracksIds] = useState<string | null>(null)
    ...
    return (
        <>
            <ul>
                {tracks.map((track) => {
                    const color = track.id === selectedTracksIds ? 'green' : 'white';
                    return (
                        <li key={track.id}>
                            <h4 style={{color}}
                                onClick={ ()=> { setSelectedTracksIds(track.id) } }
                            >
